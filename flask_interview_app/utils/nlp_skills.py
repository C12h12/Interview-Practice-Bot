...
# -------------------------------
# 2) Load NLP + embedder once
# -------------------------------
# Load at import time (for Flask, no caching decorator)
...
def generate_candidates(text: str, nlp):
    ...
    bad = {"experience", "project", "projects", "knowledge", "skills",
           "framework", "tools", "technology", "technologies", "years"}
...
# -------------------------------
def extract_skills_smart(text, catalog=None,
                         fuzzy_cutoff=90, semantic_threshold=0.72):
    if catalog is None:
        catalog = BASE_SKILLS

    cands = generate_candidates(text, nlp)
    return smart_match_skills(
        cands, catalog, embedder,
